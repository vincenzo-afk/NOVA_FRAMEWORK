...
sequenceDiagram
    ...
    MR->>CB: Check provider health
    CB-->>MR: Primary: Open (unhealthy)
    MR->>P2: Route to fallback
    P2--xMR: Timeout
    MR->>CB: Record failure, re-check chain
    alt Chain exhausted
        MR-->>PL: NOVA-AI001: all providers exhausted
    else Next fallback available
        MR->>P1: Retry primary (half-open trial)
    end
```
