...
| Keyboard / Mouse | Event-based (activity-only signal, per the scoping above) | Sampled at low frequency for idle-detection purposes only — this is the one source using sampling rather than pure event-driven capture, since "idle" is inherently a duration-based judgment, not a discrete event |
