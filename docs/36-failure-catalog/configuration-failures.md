# Configuration Failures

## Known failure patterns

Invalid config accepted without validation, failing later at point-of-use with a confusing error; environment-specific config accidentally shipped to the wrong environment.

## Cross-reference

See the corresponding subsystem file in `45-code-perfection-failure-modes/` for the code-level prevention checklist, and `25-failure-modes/INDEX.md` for the full narrative failure-mode set.
