...
|---|---|---|---|---|---|---|
| **FM-25-008** | `nova env`'s report disagrees with the live Capability Registry | Detection is cached/stale in one but not the other (same root cause as `FM-04-004`, ignore-local-model). | Cross-check test compares `nova env --json` output against a live Capability Registry query. | Medium | Both should read from the same underlying `docs/18-providers/hardware-detection.md` detection call, not maintain separate caches. | Refresh whichever side is stale; converge on a single detection source if the drift recurs. |
