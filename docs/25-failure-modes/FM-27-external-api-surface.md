...
| **FM-27-007** | Internal API accidentally exposed or reachable externally | `internal-api.md`'s UI-to-Gateway surface (which deploys in lockstep and is exempt from the external versioning policy) becomes reachable from outside the local machine due to a networking/configuration mistake. | A port/network scan or security review finds the internal API surface listening on a non-loopback interface. | Critical | The internal API is bound to loopback/local-IPC only by construction, never configurable to bind externally, distinct from the REST/WebSocket APIs which are the intentionally external-facing surface. | Treat as a security incident per `docs/10-security/threat-model.md`; audit for any external access that occurred while exposed; fix the binding and add a startup check that refuses to start if the internal API would bind non-locally. |
