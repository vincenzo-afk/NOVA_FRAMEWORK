...
| **FM-02-005** | Create impossible plans | Plan references a capability/tool that does not exist or is disabled. | Capability lookup fails at plan-validation time (`docs/18-providers/capability-management.md`). | Medium | Validate every planned tool call against the live capability registry before committing to the plan. | Replan using an available substitute capability, or surface to the user that the request cannot be fulfilled. |
...
| **FM-02-012** | Starvation | Low-priority tasks never execute because high-priority tasks keep preempting the scheduler. | Task age metric for low-priority queue grows unbounded. | Medium | Aging-based priority boost so no task waits indefinitely, per `docs/03-runtime/job-scheduler.md`. | Force-schedule the oldest starved task ahead of a same-priority-tier new task. |
