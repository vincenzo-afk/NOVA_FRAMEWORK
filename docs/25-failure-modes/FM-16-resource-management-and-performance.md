...
| **FM-16-009** | Slow memory search | Retrieval latency degrades as the memory corpus grows, especially with an unoptimized or unindexed similarity search. | Search-latency metric trends upward with corpus size beyond the expected sublinear/logarithmic curve. | Medium | Use an appropriately-scaled ANN index (not brute-force) once corpus size crosses a threshold, per `docs/04-memory/retrieval-engine.md`. | Rebuild with a more scalable index structure; consider sharding for very large corpora. |
