...
The failures severe enough to warrant their own incident-response runbook rather than a routine mitigation table entry. Every entry here should have a corresponding named runbook in `docs/13-devops/incident-response.md` and `docs/13-devops/runbook.md` — this file is the index and cross-reference, not a substitute for those runbooks.
...
| **FM-21-010** | OS crash | Underlying operating system crashes/panics, taking all NOVA processes down with it. | OS-level crash log / unexpected full-system reboot. | High | Same graceful-startup/resume discipline as FM-15 applied at the OS-crash-recovery level; nothing NOVA-specific should assume the OS never crashes. | Rely on FM-15's startup/resume-after-crash mitigations to bring services back cleanly; investigate the OS-level crash cause separately (may be hardware, driver, or resource-exhaustion related). |
