...
| **FM-03-007** | Produce inconsistent outputs | Same sub-task run twice (e.g. after a retry) produces materially different results due to non-determinism. | Diff between two runs of the same sub-task with identical input exceeds an expected-variance threshold. | Medium | Pin model temperature/seed for deterministic sub-tasks (code generation, structured extraction) per `docs/05-ai/deterministic-first.md`. | Prefer the verified/tested output; discard the other; investigate the non-determinism source. |
