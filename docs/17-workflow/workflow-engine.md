...
- **Parallel split / Join** — concurrent execution of independent
  ...
  reaches it, and per `docs/11-performance/concurrency.md`, branches
  requiring the same resource lock are still serialized against each
  other by the Resource Manager even when modeled as "parallel" in the
  workflow graph.
...
exists — bounding the entire graph's total execution time, independent of
individual node timeouts — since a workflow with many branches could
otherwise complete each node within its own budget while the overall
workflow runs far longer than intended.
