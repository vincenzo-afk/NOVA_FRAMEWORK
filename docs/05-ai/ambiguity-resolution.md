...
2. If exactly one high-confidence match exists, it is returned directly —
   no LLM involved.
3. If multiple equally plausible matches exist (e.g., "resume.pdf" in two
   different project folders), the LLM is invoked specifically to rank
   candidates using available context (recency, project relevance, past
   usage) — this is the "multiple valid candidates" branch.
