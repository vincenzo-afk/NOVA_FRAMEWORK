...
- [ ] No function added calls an LLM provider directly, bypassing
      `05-ai/model-router.md` (unless the task *is* the Model Router).
- [ ] No function added writes to a memory tier bypassing
      `04-memory/memory-storage.md`'s write path.
...
- [ ] Every new cross-device or cross-process message is schema-versioned
      per `08-api/schemas.md` and `28-multi-device-protocol/`.
- [ ] Every new error case is added to
      `26-system-reference/06-error-catalog.md` rather than left as an
      ad-hoc string.
- [ ] Every new state machine or status field is added to
      `26-system-reference/04-state-transition-tables.md`.
- [ ] Tests exist per `12-testing/testing-strategy.md` for the layer(s)
