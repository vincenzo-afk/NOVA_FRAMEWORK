...
2. **Every documented failure mode from `25-failure-modes/` and
   `45-code-perfection-failure-modes/` relevant to this feature** — each
...
4. **Permission/security** — the action is denied when the actor lacks
   permission, per `10-security/permissions.md`.
...
6. **Observability** — the action emits the event(s) documented in
   `35-analytics/events.md` and/or `26-system-reference/07-event-catalog.md`.
