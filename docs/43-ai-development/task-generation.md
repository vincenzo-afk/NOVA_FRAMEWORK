...
3. **Every task that touches a high fan-in component
   (`dependency-map.md`) gets an explicit "consumers affected" field**
...
4. **Every task references the failure modes it must handle.** Pull the
   relevant entries from `25-failure-modes/` and
   `45-code-perfection-failure-modes/` for the subsystem and attach them
...
- Writing "handle errors appropriately" as an acceptance criterion instead
  of enumerating the specific errors from `26-system-reference/06-error-catalog.md`.
